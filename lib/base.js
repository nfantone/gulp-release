...
var gitflowBase = (function() {
  ...
  GitflowBase.prototype.push = function push(ref, opts, done) {
    git.push('origin', ref, {
      args: opts,
      quiet: true
    }, done);
  ...
  GitflowBase.prototype.tag = function tag(done) {
    ...
    git.tag(version, 'tagging release ' + version, {
      quiet: true
    }, done);
  ...
  GitflowBase.prototype.commit = function commit(message) {
    return gulp.src('.')
      .pipe(git.commit('[gulp-gitflow] ' + util.format(message, [self.config.packages]), {
        args: '-a',
        quiet: true
      }));
  };
